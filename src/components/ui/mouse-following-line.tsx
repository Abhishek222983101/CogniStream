 ...
 * Adjusted for the brutalist theme without relying on next-themes.
 */
export function GridAnimation({
  ...
  strokeLength = 15,
  strokeWidth = 2,
  lineColor = "rgba(0,0,0,0.15)", // Default to subtle black for our cream background
  ...props
}: GridAnimationProps) {
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* We make the canvas very large to cover the whole section */}
      <canvas 
        ref={canvasRef} 
      ...
      <motion.div
        ref={ballRef}
        className="absolute w-[8px] h-[8px] rounded-full bg-black shadow-[2px_2px_0px_rgba(0,0,0,0.5)] z-10 pointer-events-none"
        style={{
          x: 0,
          y: 0,
          marginLeft: -4, // Center the ball
          marginTop: -4,
        }}
      />
    </div>
